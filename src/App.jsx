...
function generateFrames({ role, intervention, audience, resistances, hasData }) {
  ...
  frames.push({
    ...
    message: `${audience === 'council' ? `Our community has a responsibility — and a legal obligation — to ensure our streets protect everyone who uses them. ${iv.evidenceLine} This investment reduces our liability exposure and positions us alongside the growing number of communities that have made this commitment to their constituents.` : audience === 'dot' ? `The evidence base for this intervention is consistent across study contexts and aligns with current FHWA and NCHRP guidance. ${iv.evidenceLine} Communities we serve deserve infrastructure that reflects what the research clearly supports.` : audience === 'schoolboard' ? `Every family sending a child to our schools deserves to know the path there was designed with that child's safety in mind. ${iv.evidenceLine} This is an investment in our students and our families — and in the peace of mind we owe them.` : audience === 'media' ? `What happened here isn't an isolated incident — it's part of a pattern our community has the tools to change. ${iv.evidenceLine} The question isn't whether the solution exists. It's whether we're willing to make the same commitment to our neighbors that other communities have made to theirs.` : `Our neighbors are counting on us to make this a community where everyone can get around safely, regardless of how they travel. ${iv.evidenceLine} When our streets work for all of us, they work better for each of us.`}`,
